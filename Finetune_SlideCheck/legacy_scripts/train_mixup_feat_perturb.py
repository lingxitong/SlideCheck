# -*- coding: utf-8 -*-
"""
SlideCheck Training Script - Baseline + Mixup + Feature Perturbation
...
def train_epoch_mixup_feat_perturb(model, loader, optimizer, device,
                                     mixup_alpha=0.4, perturb_sigma=0.1,
                                     lambda_constraint=1.0, pos_weight_abn=None, pos_weight_can=None):
    """Mixup + Feature Perturbation"""
    model.train()
    total_loss = 0
    ...
    for embeddings, na_labels, cc_labels in tqdm(loader, desc='Train', leave=False):
        ...
        perturbed_emb = embeddings * (1 + noise)

        # ===== Step 2: Mixup =====
        lam = np.random.beta(mixup_alpha, mixup_alpha)
        lam = max(lam, 1 - lam)
        index = torch.randperm(batch_size, device=device)
        mixed_emb = lam * perturbed_emb + (1 - lam) * perturbed_emb[index]
...
def main():
    ...
    parser.add_argument('--exp_name', type=str, default='exp_mixup_feat_perturb')
    ...
    print(f"Mixup alpha: {args.mixup_alpha}, FeatPerturb sigma: {args.perturb_sigma}")
    ...
    print(f"\n{'='*60}")
    print("开始训练 (Mixup + Feature Perturbation)")
    print(f"{'='*60}\n")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        # 训练 - 使用Mixup
        train_loss = train_epoch_mixup_feat_perturb(model, train_loader, optimizer, args.device,
